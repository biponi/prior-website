...
function CategoryMegaMenu({ categories }: { categories: Category[] }) {
  ...
  return (
    <div className='flex w-[760px] rounded-xl overflow-hidden shadow-2xl border border-[#f0e0eb] bg-white'>
      ...
      <div className='w-48 bg-gradient-to-b from-[#fdf2f7] to-[#fce8f2] py-3 flex-shrink-0'>
        ...
        {categories.map((cat) => (
          <Link
            ...
            onMouseEnter={(e) => {
              // Prevent navigation on hover — just highlight
              e.preventDefault();
              setActiveParent(cat);
              setActiveChild(null);
            }}
            onClick={(e) => {
              // Allow click to navigate directly
              // But we keep the hover highlight too
            }}
            className={cn(
      ...
      {/* Column 2 – Sub-categories or direct CTA */}
      <div className='flex-1 border-x border-[#f0e0eb] py-4 px-4'>
        {activeParent && (
          <>
            {/* Header: parent name + direct link */}
            <div className='flex items-center justify-between mb-3'>
              <div>
                <Link
                  href={`/category/${activeParent.slug}`}
                  className='font-semibold text-gray-800 hover:text-[#CD2A75] transition-colors'>
                  {activeParent.name}
                </Link>
              ...
              <>
                ...
                <div className='mt-3 pt-3 border-t border-[#f0e0eb]'>
                  <Link
                    ...
                    Browse all {activeParent.name}{" "}
                    <ArrowRight size={14} className='ml-1.5' />
                  </Link>
                </div>
              </>
            ) : (
              /* No children: show a clean, inviting CTA instead of empty space */
              <div className='flex flex-col items-center justify-center py-6 gap-4'>
                <div className='w-14 h-14 rounded-2xl bg-[#fdf2f7] flex items-center justify-center'>
                  {activeParent.img ? (
                    <img
                      src={activeParent.img}
                      alt={activeParent.name}
                      className='w-10 h-10 rounded-xl object-cover'
                    />
                  ) : (
                    <Layers size={24} className='text-[#CD2A75]' />
                  )}
                </div>
                <div className='text-center'>
                  ...
                  {activeParent.totalProducts > 0 && (
                    <p className='text-xs text-gray-400 mt-0.5'>
                      {activeParent.totalProducts} products
                    </p>
                  )}
                </div>
                <Link
                  href={`/category/${activeParent.slug}`}
                  className='inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-[#CD2A75] hover:bg-[#b02165] rounded-lg transition-colors shadow-sm shadow-[#CD2A75]/20'>
      ...
      {/* Column 3 – Grand-children or quick links */}
      <div className='w-44 py-4 px-3 flex flex-col gap-3 flex-shrink-0'>
        {activeChild && grandChildren.length > 0 ? (
          ...
          /* Quick links when no grandchildren */
          <div className='flex flex-col gap-2'>
            ...
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className='flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs text-gray-600 hover:bg-[#fdf2f7] hover:text-[#CD2A75] transition-colors group'>
                <link.icon
                  size={12}
                  className='text-[#CD2A75]/40 group-hover:text-[#CD2A75] transition-colors'
                />
            ...
            <div className='mt-auto pt-2 border-t border-[#f0e0eb]'>
              <Link
                href='/collections'
                className='flex items-center justify-center w-full px-3 py-2 text-[11px] font-medium text-[#CD2A75] bg-[#fdf2f7] hover:bg-[#fce8f2] rounded-lg transition-colors'>
                View all products <ArrowRight size={11} className='ml-1' />
              </Link>
            </div>
          </div>
        )}
        ...
        <div
          ...
          <Link
            ...
            className={cn(
              ...
              depth === 0 ? "font-medium text-gray-700" : "text-gray-500",
